...
# New session call
Bash({
  command: "node scripts/dispatch/run.js {{LITE_MODE_FLAG}}--backend codex - \"$PWD\" <<'EOF'
...
# Resume session call
Bash({
  command: "node scripts/dispatch/run.js {{LITE_MODE_FLAG}}--backend codex resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
| Phase | Codex |
|-------|-------|
| Analysis | `scripts/dispatch/prompts/codex/analyzer.md` |
| Planning | `scripts/dispatch/prompts/codex/architect.md` |
| Review | `scripts/dispatch/prompts/codex/reviewer.md` |
